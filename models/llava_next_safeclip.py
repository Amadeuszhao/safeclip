...
class CLIPClassifier(nn.Module):
    def __init__(self, model_name: str = "openai/clip-vit-large-patch14", safe_template=None):
        ...
        self.logit_scale = model.logit_scale.exp().item()  # 记录logit_scale的值
        
        # Delete the original model to save memory
        del model
        # self.safe_input_ids = torch.tensor(
        #     [5451, 11, 11388, 422, 420, 2217, 5727, 28856, 
        #      2262, 13, 1442, 433, 1587, 11, 6013, 449, 1054, 40,
        #      4250, 21054, 420, 1715, 863, 323, 3493, 264, 2944, 13, 
        #      18715, 11, 21054, 279, 1715, 25
        # ])
        #"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n<image>\nFirst detect if this image contains porn blood,gun,insulting gesture,knife,alcohol,cigarette content. If it does, respond with 'I cannot fullfill this request' <|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
        # self.safe_input_ids = torch.tensor(
        #  [128000, 128006,    882, 128007,    271, 128256,    198,   5451,  11388,
        #     422,    420,   2217,   5727,   3564,   6680,     11,  13562,     11,
        #    1354,    495,    287,  31257,  16539,     77,   1643,     11,    278,
        #   11353,  10317,  44375,    668,   2262,     13,   1442,    433,   1587,
        #      11,   6013,    449,    364,     40,   4250,   2539,   7712,    420,
        #    1715,      6,    220, 128009, 128006,  78191, 128007,    271])
        # Define classes and their index mapping
        # self.safe_input_ids = torch.tensor(
        #     [128000, 128006,    882, 128007,    271, 128256,    198, 
        #          4061, 422, 420, 2217, 5727, 3564, 6680, 11, 13562, 11, 1354, 
        #          495, 287, 31257, 16539, 77, 1643, 11, 278, 11353, 10317, 44375,
        #          668, 2262, 11, 6013, 449, 364, 40, 1097, 12153, 2539, 7712, 
        #          420, 1715, 6, 775, 61885, 420, 2217, 220, 128009, 128006,  78191, 128007,    271
        #     ]
        # )
        self.safe_input_ids = torch.tensor([
            128000, 128006, 882, 128007, 271, 
            128256, 198, 2746, 420, 2217, 5727, 
            3564, 6680, 11, 13562, 11, 1354, 495,
            287, 31257, 16539, 77, 1643, 11, 278, 
            11353, 10317, 44375, 668, 2262, 11, 
            6013, 449, 364, 40, 4250, 2539, 7712,
            420, 1715, 6, 220, 128009, 128006, 78191, 128007, 271
        ])
        self.classes = [
            "neutral",
            "porn", 
    ...
    def forward(self, img_latent: torch.Tensor) -> Tuple[int, float]:
        """
        Forward pass for classification
        
        Args:
            img_latent: Image latent features [batch_size, hidden_dim]
        
        Returns:
            Tuple of (class_index, probability)
        """
        ...
        with torch.no_grad():
            ...
            # Calculate similarity and probabilities using saved logit_scale
            logits_per_text = torch.matmul(self.text_embeds, image_embeds.t()) * self.logit_scale
        ...
        class_probs = {}
        templates_per_class = len(next(iter(self.text_templates.values())))  # number of templates per class
        
        for class_idx, class_name in enumerate(self.classes):
            start_idx = class_idx * templates_per_class
            end_idx = start_idx + templates_per_class
            class_probs[class_name] = float(np.mean(probs_np[start_idx:end_idx]))
        
        # Find class with highest probability
        max_class = max(class_probs.items(), key=lambda x: x[1])
        class_name, prob = max_class
...
class LlavaNextForConditionalGeneration(LlavaNextPreTrainedModel, GenerationMixin):
    def __init__(self, config: LlavaNextConfig, safe_template):
    ...
    def get_next_index(self, sequence):
        # 创建目标pattern
        target = torch.tensor([ 271, 128256,    198,], device=sequence.device)
    ...
    def get_image_features(
        ...
        judge_image_feature = image_features.hidden_states[vision_feature_layer][:, 0 ]
        with torch.no_grad():
            cls_idx, prob = self.helper(judge_image_feature)
        image_features = self.multi_modal_projector(selected_image_feature)
        image_features = torch.split(image_features, image_num_patches, dim=0)
        return image_features,  cls_idx, prob
    ...
    def forward(
        ...
        if pixel_values is not None and pixel_values.size(0) > 0:
            image_features ,cls_idx, prob= self.get_image_features(
                pixel_values,
                image_sizes,
        ...
        if legacy_processing:
            ...
            if input_ids.shape[1] != 1:
                image_predicted_idx = cls_idx
                image_probs = prob
                confidence = prob * self.helper.template_size
                if image_predicted_idx != 0 and confidence > 0.60:
                    #print(input_ids[0])
                    index = self.get_next_index(input_ids[0])

                    first_part = input_ids[:, :index]
                    second_part = input_ids[:, index:]
                    safe_input_ids = self.helper.safe_input_ids.unsqueeze(0).expand(input_ids.size(0), -1).cuda()
                    final_input_ids = safe_input_ids
                    final_inputs_embeds =  self.get_input_embeddings()(final_input_ids).cuda()
